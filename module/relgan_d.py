...
dis_num_filters = [128, 128, 128, 128]
...
class RelSpaceGAN_D(CNNDiscriminator):
    def __init__(self, embed_dim, max_seq_len, num_rep, vocab_size, padding_idx, kbins, gpu=False, dropout=0.25):
        super(RelSpaceGAN_D, self).__init__(embed_dim, vocab_size, dis_filter_sizes, dis_num_filters, padding_idx,
                                       gpu, dropout)
        ...
        self.feature2out = nn.Linear(self.feature_dim, 100)
        self.out2logits = nn.Linear(100, 1)
        self.embed2bins = nn.Linear(num_rep, kbins)
        self.out2latent = nn.Linear(num_rep, 100)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, inp):
        ...
        highway = self.highway(pred)
        pred = torch.sigmoid(highway) * F.relu(highway) + (1. - torch.sigmoid(highway)) * pred  # highway
        pred = F.relu(self.feature2out(self.dropout(pred)))

        logits = self.out2logits(pred).squeeze(1)  # [batch_size * num_rep]

        latent = pred.reshape(batch_size, -1, self.num_rep).sum(1)
        latent = F.relu(latent)
        kbins = self.embed2bins(latent)
        latent = self.out2latent(latent)

        return logits, kbins, latent


class RelSeqGAN_D(CNNDiscriminator):
    def __init__(self, embed_dim, max_seq_len, num_rep, vocab_size, padding_idx, prod_size, gpu=False, dropout=0.25):
        super(RelSeqGAN_D, self).__init__(embed_dim, vocab_size, dis_filter_sizes, dis_num_filters, padding_idx,
                                       gpu, dropout)
        ...
        self.feature2out = nn.Linear(self.feature_dim, 100)
        self.out2logits = nn.Linear(100, 1)
        self.embed2bins = nn.Linear(num_rep, prod_size)
        # self.out2latent = nn.Linear(num_rep, 100)
        self.dropout = nn.Dropout(dropout)
