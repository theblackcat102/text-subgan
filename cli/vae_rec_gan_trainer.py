...
class TemplateTrainer():
    ...
    def __init__(self, args):
        ...
        self.P = ProductHead(self.D.feature_dim, 64)

        self.D.cuda(), self.P.cuda()
        ...
        self.dis_adv_opt  = optim.Adam(list(self.D.parameters())+list(self.P.parameters()), lr=args.dis_lr, betas=(0.5, 0.999))

        opt_level = args.opt_level
        # [self.model, self.C], self.gen_adv_opt = amp.initialize([self.model, self.C], self.gen_adv_opt, opt_level=opt_level)

        self.dataloader1 = torch.utils.data.DataLoader(self.dataset1, num_workers=1,
                        collate_fn=seq_collate, batch_size=args.batch_size, shuffle=True, drop_last=True)

        self.dataloader2 = torch.utils.data.DataLoader(self.dataset2, num_workers=1,
                        collate_fn=seq_collate, batch_size=args.batch_size, shuffle=True, drop_last=True)
    ...
    def dis_step(self, i):
        ...
        pos_index = torch.bmm( item_.unsqueeze(1), items1.unsqueeze(2) ).sigmoid()
        neg_index = torch.bmm(item_.unsqueeze(1), neg_items.unsqueeze(2)).sigmoid()

        item_loss = ((pos_index - 1)**2 + neg_index**2 ).sum() / pos_index.shape[0]

        # item_loss += self.mse_criterion(item_, items1) + (torch.bmm(new_item_.unsqueeze(1), neg_items.unsqueeze(2)) ).sigmoid().sum()
        ...
        loss = d_loss + cond_loss + item_loss
        loss.backward()
        torch.nn.utils.clip_grad_norm_(self.P.parameters(), cfg.clip_norm)
        torch.nn.utils.clip_grad_norm_(self.D.parameters(), cfg.clip_norm)

        self.dis_adv_opt.step()

        return loss.item(), d_loss.item(), cond_loss.item(), item_loss.item()
    ...
    def step(self, i):
        ...
        fake_item_ = self.P(fake_latent)

        desc_kl_loss += self.KL_loss(desc_m, desc_v)

        pos_index = torch.bmm( fake_item_.unsqueeze(1), users2.unsqueeze(2) ).sigmoid() 
        neg_index = torch.bmm(fake_item_.unsqueeze(1), neg_items2.unsqueeze(2)).sigmoid()
        item_loss = self.mse_criterion(fake_item_, items2) + self.mse_criterion( item_, items1 )
        item_loss += ((pos_index-1)**2 + neg_index**2).sum() / pos_index.shape[0]
        ...
        total_loss = reconstruct_loss*self.args.re_weight + \
                ...
                g_loss_ * self.args.dis_weight + item_loss
        ...
        torch.nn.utils.clip_grad_norm_(self.Q.parameters(), cfg.clip_norm)
        ...
        return total_loss.item(), total_kl_loss.item(), reconstruct_loss.item(), 0, desc_kl_loss.item(), template_loss.item(), g_loss_.item(), item_loss.item()
    ...
    def train(self):
        ...
        with tqdm(total=args.iterations+1, dynamic_ncols=True) as pbar:
            for i in range(args.iterations+1):
                ...
                total_loss, total_kl_loss, reconstruct_loss, cycle_nll_loss, title_cycle_nll, tmp_cycle_nll, g_loss, g_bpr_loss = self.step(i)
                self.model.eval(), self.D.train(), self.Q.eval()
                d_loss, logit_loss, mle_loss, d_bpr_loss = self.dis_step(i)
                ...
                pbar.set_description(
                    'g_loss: %.4f, c_loss: %.4f, bpr: %.4f' % (total_loss, reconstruct_loss, d_bpr_loss))
